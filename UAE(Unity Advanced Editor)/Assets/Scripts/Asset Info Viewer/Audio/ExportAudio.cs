...
public class ExportAudio : MonoBehaviour
{
                                    ...
                                    {
                                        {
                                            ...
                                            if (streamInfo.path != null && streamInfo.path != "")
                                            {
                                                ...
                                                else
                                                {
                                                    ...
                                                    StartCoroutine(SaveAudio(wavbytes));
                                                }
                                            }
                                            else
                                            {
    ...
    public void ExportAudioRest(byte[] resourceBytes)
    {
        if (selectedFieldInfo.cell != null)
        {
            ContactInfo info = selectedFieldInfo.cell._contactInfo;
    ...
    IEnumerator SaveAudio(byte[] wavBytes)
    {
        yield return FileBrowser.WaitForSaveDialog(FileBrowser.PickMode.FilesAndFolders, false, null, null, "Save Files and Folders", "Save");
        try
        {
            string path = FileBrowser.Result[0];
            File.WriteAllBytes(path, wavBytes);

            load.ComplainError("Exported Audio Successfully.");
        }
        catch (Exception ex)
        {
